...
interface TsStatus {
  Self?: TsStatusPeer;
  Peer?: Record<string, TsStatusPeer>;
}
 ...
 */
export async function magicDnsFor(peerName: string): Promise<string | null> {
  const now = Date.now();
...
async function loadTailscaleMap(): Promise<Map<string, string>> {
  const map = new Map<string, string>();
  const raw = await runTailscaleStatus();
  if (!raw) return map;
  let status: TsStatus;
  try { status = JSON.parse(raw) as TsStatus; }
  catch { return map; }
  const all: TsStatusPeer[] = [];
  if (status.Self) all.push(status.Self);
  if (status.Peer) for (const p of Object.values(status.Peer)) all.push(p);
  for (const p of all) {
    const fqdn = (p.DNSName ?? "").replace(/\.$/, "");
    if (!fqdn) continue;
    const host = (p.HostName ?? "").toLowerCase();
    const shortName = fqdn.split(".")[0]!.toLowerCase();
    if (host) map.set(host, fqdn);
    if (shortName) map.set(shortName, fqdn);
  }
  return map;
}
...
function runTailscaleStatus(): Promise<string | null> {
  return new Promise((resolve) => {
    ...
    setTimeout(() => { try { child.kill("SIGTERM"); } catch { /* */ } resolve(null); }, 2000);
  });
}

/** Test seam: forget the cached map so the next lookup re-shells out. */
export function _resetTailscaleCache(): void { cached = null; }
