...
import { loadOrGenerateHubCert, type HubCert } from "./cert.js";
 ...
 */
export async function runHub(opts: HubOptions): Promise<RunningHub> {
  ...
  const server = createHttpsServer(
    ...
    async (req, res) => {
      try {
        ...
        log(`web: handler error: ${(err as Error).message}`);
        if (!res.headersSent) {
  ...
  server.on("upgrade", (req, socket, head) => {
    try {
      ...
      log(`web: upgrade error: ${(err as Error).message}`);
      socket.destroy();
    }
  });
...
async function handleMintToken(
  ...
  let body: { node?: unknown; session?: unknown; readonly?: unknown };
  try {
  ...
  const sessions = await collectNetSessions(ctx.nodeName);
  const allowed = sessions.some(
    (s) => !("unreachable" in s) && s.node === node && s.session.name === session,
  );
  if (!allowed) {
    res.writeHead(404, { "content-type": "text/plain" });
  ...
  const token = mintToken(
    { node, session, iat: now, exp: now + SPA_TOKEN_TTL_MS, ro: !!body.readonly },
    ctx.hmacSecret,
  );
  res.writeHead(200, { "content-type": "application/json" });
...
async function collectNetSessions(selfNodeName: string): Promise<Array<NetSessionRow | UnreachableRow>> {
  ...
  const rows: Array<NetSessionRow | UnreachableRow> = merged.rows.map(({ node, session }) => ({
    node,
    session,
...
async function listLocalSessions(nodeName: string): Promise<ListResultEntry[]> {
  ...
  try {
    ...
    if (helloOk.verb !== "OK") return [];
    const r = await client.request({ verb: "LIST", filter: { all: true } });
    if (r.verb !== "LIST_RESULT") return [];
    return (r as { sessions: ListResultEntry[] }).sessions;
  } finally {
    client.close();
  }
}
