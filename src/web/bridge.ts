...
export interface BridgeOptions {
  ...
  /**
   * When the requested peerNode equals the hub's own node name, skip the
   * ssh hop and spawn ttyd directly. This keeps "hub serves its own
   * sessions" working without requiring a `Host nagent.<self>` ssh_config
   * entry (which we don't write).
   */
  selfNodeName: string;
}
...
export async function openTtydBridge(opts: BridgeOptions): Promise<void> {
  ...
  if (isSelfHosted) {
    ...
    const child = spawn(
      ...
      ["-ilc", `rm -f ${shellSingleQuote(localSock)}; ${remoteCmd}`],
