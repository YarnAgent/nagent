...
function parse(content: string): ParsedFile {
  ...
  const out: ParsedFile = { before: [], inside: [], after: [], hadFence: false };
  let phase: "before" | "inside" | "after" = "before";
  for (const line of lines) {
    if (phase === "before") {
      if (line === FENCE_OPEN) { phase = "inside"; out.hadFence = true; continue; }
      out.before.push(line);
    } else if (phase === "inside") {
      if (line === FENCE_CLOSE) { phase = "after"; continue; }
...
function render(p: ParsedFile): string {
  const parts: string[] = [];
  if (p.before.length) parts.push(p.before.join("\n"));
  parts.push(FENCE_OPEN);
  if (p.inside.length) parts.push(p.inside.join("\n"));
  parts.push(FENCE_CLOSE);
  if (p.after.length) parts.push(p.after.join("\n"));
...
function lineWithTag(line: string, tag: string): string {
  return `${line.trim()}  # nagent-tag=${tag}`;
}

function tagOf(line: string): string | undefined {
...
async function writeKeysFile(path: string, content: string): Promise<void> {
...
export async function appendAuthorizedKey(args: {
  ...
  if (idx >= 0) parsed.inside[idx] = tagged;
  else parsed.inside.push(tagged);
  await writeKeysFile(path, render(parsed));
}

/** Remove the line with the given tag. No-op if not present. */
export async function removeAuthorizedKey(args: { tag: string; path?: string }): Promise<boolean> {
  const path = args.path ?? authorizedKeysPath();
  const content = await readFileOrEmpty(path);
  ...
  if (parsed.inside.length === before && !parsed.hadFence) return false;
  await writeKeysFile(path, render(parsed));
  return parsed.inside.length !== before;
}

/** True if a line with the given tag exists in the fenced block. */
export async function hasAuthorizedKeyTag(tag: string, path?: string): Promise<boolean> {
