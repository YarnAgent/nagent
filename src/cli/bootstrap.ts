...
export interface BootstrapOptions {
  /** Skip the daemon-spawn step (e.g. inside the `daemon` subcommand). */
  skipDaemon?: boolean;
  /** Output sink for the "nagent: ..." log lines. Defaults to stderr. */
...
export async function bootstrap(opts: BootstrapOptions = {}): Promise<void> {
  if (process.env[BOOT_FLAG] === "1") return;
  ...
  try {
    ...
    if (!opts.skipDaemon) await ensureDaemon(log);
  } finally {
    await releaseLock();
  }
}

export async function ensureIdentity(log: (line: string) => void): Promise<void> {
...
export async function ensureDefaultNet(log: (line: string) => void): Promise<void> {
...
export async function ensureDaemon(log: (line: string) => void): Promise<void> {
  if (await socketIsLive()) return;
...
async function waitForSocket(maxMs: number): Promise<boolean> {
  const deadline = Date.now() + maxMs;
  while (Date.now() < deadline) {
    if (await socketIsLive()) return true;
    await sleep(SOCKET_POLL_MS);
  }
  return false;
}

async function readPidIfStale(): Promise<number | undefined> {
  try {
