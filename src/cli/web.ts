...
export async function cmdWebStop(): Promise<void> {
  ...
  try {
    ...
    throw new Error(`failed to stop hub pid ${rec.pid}: ${(err as Error).message}`);
  }
}

// ----- `nagent web token` -----

export interface WebTokenOpts {
  session: string;          // "<node>/<sess>" required
  expires?: string;          // "1h", "30m", "120" (seconds), etc.
  readonly?: boolean;
}

const DEFAULT_TOKEN_TTL_MS = 60 * 60 * 1000; // 1h

export async function cmdWebToken(opts: WebTokenOpts): Promise<void> {
  ...
  if (!node || !session) throw new Error(`invalid --session: ${opts.session}`);
  const ttlMs = parseTtl(opts.expires) ?? DEFAULT_TOKEN_TTL_MS;
  const now = Date.now();
  const config = await loadOrGenerateWebConfig();
  const token = mintToken(
    { node, session, iat: now, exp: now + ttlMs, ro: !!opts.readonly },
    config.hmacSecret,
  );
...
function parseTtl(input: string | undefined): number | undefined {
  ...
  if (!m) throw new Error(`bad --expires: ${input} (use 30s / 5m / 1h / 7d)`);
...
interface KnownHubs {
  v: 1;
  hubs: Record<string, { fingerprint: string; pinnedAt: string }>;
}

export async function cmdWebTrust(opts: WebTrustOpts): Promise<void> {
  if (!opts.hubUrl) throw new Error("usage: nagent web trust <hub-url>");
  const u = new URL(opts.hubUrl);
  if (u.protocol !== "https:") throw new Error(`hub URL must be https:// (got ${u.protocol})`);
  ...
  let store: KnownHubs;
  try {
    store = JSON.parse(await fs.readFile(path, "utf8")) as KnownHubs;
    if (store.v !== 1 || typeof store.hubs !== "object") throw new Error("bad known_hubs.json");
  } catch {
    store = { v: 1, hubs: {} };
  }
