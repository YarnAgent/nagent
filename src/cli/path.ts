...
export interface PathStatusOpts {
  peer?: string;
  json?: boolean;
}

export async function cmdPathStatus(opts: PathStatusOpts): Promise<void> {
...
function renderTable(t: PathTable): void {
  if (Object.keys(t.direct).length === 0 && Object.keys(t.relays).length === 0) {
    process.stdout.write(
      `path-table is empty. Run \`nagent path probe\` to populate it, or wait for the daemon's periodic tick.\n`,
    );
    return;
  }
  ...
  if (directRows.length === 0) process.stdout.write("  (no direct probes)\n");
  for (const [peer, ms] of directRows) process.stdout.write(`  ${peer.padEnd(24)} ${ms}\n`);

  // Relays
  process.stdout.write("\nRELAYS\n");
  if (Object.keys(t.relays).length === 0) process.stdout.write("  (no pinned relays)\n");
...
function renderPeer(t: PathTable, peer: string): void {
...
export async function cmdPathProbe(): Promise<void> {
  const active = await readActiveState();
  if (!active.activeNetId) throw new Error("no active net");
  const id = await readIdentity();
  if (!id?.nodeName) throw new Error("identity missing — run `nagent` once to bootstrap");
