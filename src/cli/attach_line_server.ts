 ...
 */
export async function cmdAttachLineServer(sessionName: string): Promise<void> {
  ...
  const fifoPath = join(
    tmpdir(),
    `nagent-pipe-${sessionId}-${randomBytes(4).toString("hex")}`,
  );
  ...
  const altScreen = spawnSync(
    TMUX,
    ["-L", TMUX_SOCKET, "display-message", "-p", "-t", target, "#{alternate_on}"],
    { encoding: "utf8" },
  ).stdout.trim();
  if (altScreen === "1") {
    process.stderr.write(
      "nagent attach-line: warning — the remote pane is in full-screen TUI mode (vim/htop/etc.).\n" +
  ...
  fifoReader.on("error", () => { /* fifo may close when we tear down; ignore */ });
 ...
 * running.
 */
async function resolveSessionId(name: string): Promise<string> {
  const active = await readActiveState();
  const client = new BusClient();
  await client.connect();
  try {
    const hello = { verb: "HELLO" as const, node: "attach-line", asCli: true };
    const ok = await client.request(hello);
    if (ok.verb !== "OK") throw new Error(`HELLO failed: ${ok.verb}`);
    const r = await client.request({ verb: "LIST", filter: { all: true } });
    if (r.verb !== "LIST_RESULT") throw new Error("LIST failed");
    const sessions = (r as { sessions: ListResultEntry[] }).sessions;
    const found = sessions.find((s) => s.name === name);
    if (!found) {
      throw new Error(
        `nagent attach-line: session "${name}" not found on this node` +
