 ...
 */
export async function applyGossipAdd(
  signed: SignedGossipAdd,
  now = new Date(),
): Promise<GossipAck | GossipReject> {
  const verified = verifyGossipAdd(signed, now);
  if (!verified.ok) return { v: 1, ok: false, error: `signature: ${verified.error}` };
  ...
  if (!identity) return { v: 1, ok: false, error: "receiver not initialized" };
  const active = await readActiveState();
  if (!active.activeNetId) return { v: 1, ok: false, error: "receiver has no active net" };
  ...
  if (!netMeta) return { v: 1, ok: false, error: `receiver active net ${active.activeNetId} missing` };
  ...
  if (existingIdx >= 0 && peers[existingIdx]!.pubKey !== newPeer.pubKey) {
    const origin = (await readJson<{ originPubKey: string }>(paths().netAuthority(active.activeNetId)))?.originPubKey;
  ...
  if (existingIdx >= 0) peers[existingIdx] = peerRecord;
  else peers.push(peerRecord);
 ...
 */
export async function cmdGossipAddPeer(): Promise<void> {
...
async function readSignedFromStdin(): Promise<SignedGossipAdd> {
  const chunks: Buffer[] = [];
  for await (const c of process.stdin) chunks.push(Buffer.isBuffer(c) ? c : Buffer.from(c));
  const body = Buffer.concat(chunks).toString("utf8").trim();
  if (!body) throw new Error("empty gossip-add payload on stdin");
  try {
    return JSON.parse(body) as SignedGossipAdd;
  } catch (err) {
    throw new Error(`bad gossip-add JSON: ${(err as Error).message}`);
  }
}
