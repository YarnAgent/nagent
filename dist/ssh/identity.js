...
export async function loadSshKeypair(nodeId) {
    ...
    if (pem.includes("-----BEGIN OPENSSH PRIVATE KEY-----")) {
        ...
        const dJwk = legacyPriv.export({ format: "jwk" });
        const xJwk = legacyPub.export({ format: "jwk" });
 ...
 */
export function opensshEd25519Pem(rawPriv32, rawPub32, comment) {
...
export function parseOpensshEd25519(pem) {
    ...
    if (!m || !m[1])
        throw new Error("openssh key: missing PEM envelope");
    ...
    off += magic.length;
    const readStr = (src, refOff) => {
        if (refOff.o + 4 > src.length)
            throw new Error("openssh key: truncated");
        const n = src.readUInt32BE(refOff.o);
        refOff.o += 4;
        if (refOff.o + n > src.length)
            throw new Error("openssh key: truncated body");
    ...
    if (cipher !== "none")
        throw new Error(`openssh key: encrypted (cipher=${cipher})`);
    readStr(blob, ref); // kdfname
    readStr(blob, ref); // kdfoptions
    if (ref.o + 4 > blob.length)
        throw new Error("openssh key: truncated numkeys");
    const numKeys = blob.readUInt32BE(ref.o);
    ref.o += 4;
    if (numKeys !== 1)
        throw new Error(`openssh key: numKeys=${numKeys} not supported`);
    readStr(blob, ref); // public key blob (we read raw pub from the inner section)
    const inner = readStr(blob, ref);
    const innerRef = { o: 0 };
    innerRef.o += 4;
    innerRef.o += 4; // checkint1, checkint2
    const keytype = readStr(inner, innerRef).toString("utf8");
    if (keytype !== "ssh-ed25519")
        throw new Error(`openssh key: keytype=${keytype} not supported`);
