...
const FENCE = (peer) => ({ open: `# >>> nagent host ${peer} >>>`, close: `# <<< nagent host ${peer} <<<` });
const INCLUDE_LINE = `Include ${nagentSshConfigPath()}`;
export function nagentSshConfigPath() {
...
async function writeFile600(path, content) {
...
export async function ensureUserSshConfigInclude() {
    const path = userSshConfigPath();
    const current = await readOrEmpty(path);
    if (current.split("\n").some((l) => l.trim() === INCLUDE_LINE.trim()))
        return;
...
function stripBlock(content, peerName) {
    const f = FENCE(peerName);
    const lines = content.split("\n");
    const out = [];
    let skip = false;
    for (const l of lines) {
        if (!skip && l.trim() === f.open.trim()) {
            skip = true;
            continue;
        }
        if (skip && l.trim() === f.close.trim()) {
            skip = false;
            continue;
        }
        if (!skip)
 ...
 */
export async function writeHostEntry(entry) {
...
export async function removeHostEntry(peerName) {
    const path = nagentSshConfigPath();
    const cur = await readOrEmpty(path);
    if (!cur)
        return;
...
function assertNoForwardAgent(text) {
...
//# sourceMappingURL=ssh_config.js.map