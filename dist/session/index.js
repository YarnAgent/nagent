 ...
 */
export function createOrAttachTmuxSession(opts) {
    ...
    if (existsCheck.status !== 0) {
        const create = spawnSync("tmux", tmuxArgs(["new-session", "-d", "-s", target]), { stdio: "inherit", env });
        if (create.status !== 0) {
...
export function applyNagentKeybinds() {
...
export function attachTmuxSession(target) {
...
export function tmuxSessionExists(target) {
...
export function checkTmuxVersion() {
    ...
    if (r.status !== 0)
        return { ok: false, reason: "tmux not found on PATH" };
    const m = /tmux\s+(?:next-)?([0-9]+(?:\.[0-9]+)?)/.exec(r.stdout.toString("utf8"));
    if (!m || !m[1])
        return { ok: false, reason: `cannot parse tmux version: ${r.stdout.toString("utf8")}` };
    const major = parseFloat(m[1]);
    if (major < 3.0)
        return { ok: false, reason: `tmux ${m[1]} too old; need >= 3.0` };
