...
import { readActiveState, readNetMeta, readProjects, } from "../store/index.js";
...
export async function runPicker(input) {
    ...
    if (r0.verb !== "OK")
        throw new Error(`HELLO failed: ${r0.verb}`);
...
async function refreshState(client) {
    ...
    const sessions = list.verb === "LIST_RESULT" ? list.sessions : [];
    return {
...
async function readIdAndActive() {
...
function render(state) {
    const lines = [];
    lines.push(`nagent — node=${state.nodeName}  net=${state.netName ?? "—"}  project=${state.projectName ?? "—"}  cwd=${shortCwd(pathResolve(process.cwd()))}`);
    lines.push("");
...
async function createSessionViaClient(client, name, projectId) {
    ...
    if (r.verb !== "SESSION_CREATED")
        throw new Error(`unexpected: ${r.verb}`);
    return r.session;
}
