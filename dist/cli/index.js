...
import { cmdRelayServe, cmdRelayStop, cmdRelayAdd, cmdRelayRemove, cmdRelayList, cmdRelayGrant, cmdRelayRevoke, cmdRelayListAllowed, cmdRelayDial, } from "./relay.js";
...
async function main() {
    ...
    relayCmd
        ...
        .action(async (name) => { await cmdRelayRemove(name); });
    relayCmd
        .command("list")
    ...
    const relayDialCmd = program
        ...
        .action(async (peer, opts) => {
        process.env.NAGENT_NO_BOOTSTRAP = "1";
        await cmdRelayDial(peer, opts);
    });
    relayDialCmd._hidden = true;
    // ---------------- v0.5: nagent path … ----------------
    ...
    pathCmd
        ...
        .action(bootstrapped(async (peer, opts) => {
