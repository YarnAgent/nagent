...
import { listNets, readActiveState, readIdentity, readNetMeta, readPeers, readProjects, writeActiveState, writeNetMeta, writePeers, } from "../store/index.js";
...
function deferredHandler(verbLabel, version) {
...
const TABLE = [
    ...
    // net
    {
        ...
        handler: async (args, out) => {
            const name = args[0];
            if (!name) {
                out.err("usage: /net create <name>");
                return {};
            }
            const id = (await readIdentity());
            if (!id) {
                out.err("no identity yet (bootstrap should have created one)");
                return {};
            }
    ...
    },
    {
        ...
        handler: async (args, out) => {
            const x = args[0];
            if (!x) {
                out.err("usage: /net switch <id|name>");
                return {};
            }
            const nets = await listNets();
            const found = nets.find((n) => n.netId === x || n.name === x);
            if (!found) {
                out.err(`unknown net: ${x}`);
                return {};
            }
    ...
    // project
    {
        ...
        handler: async (args, out) => {
            const name = args[0];
            if (!name) {
                out.err("usage: /project init <name>");
                return {};
            }
            const state = await readActiveState();
            if (!state.activeNetId) {
                out.err("no active net");
                return {};
            }
            const id = (await readIdentity());
            try {
    ...
    },
    {
        ...
        handler: async (args, out) => {
            const x = args[0];
            if (!x) {
                out.err("usage: /project switch <id|name>");
                return {};
            }
            const state = await readActiveState();
            if (!state.activeNetId) {
...
export async function dispatchSlash(line, out) {
    ...
    const sub = tokens[1];
    const entry = (sub && TABLE.find((e) => e.verb === verb && e.sub === sub)) ||
...
function parseInviteArgs(args) {
    const out = {};
    for (let i = 0; i < args.length; i++) {
        const a = args[i];
        if (a === "--expires" && args[i + 1]) {
            out.expires = args[++i];
            continue;
        }
        if (a === "--addr" && args[i + 1]) {
            (out.addr ||= []).push(args[++i]);
            continue;
        }
        if (a === "--tag" && args[i + 1]) {
            (out.tag ||= []).push(args[++i]);
