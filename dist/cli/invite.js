...
import { encodeToken, generateOneTimeKey, newInviteId, newNonce, } from "../invite/index.js";
import { appendAuthorizedKey, } from "../ssh/authorized_keys.js";
import { sshAuthorizedKeysLine, loadSshKeypair, } from "../ssh/identity.js";
import { currentReachableAddresses, parseAddressArg, } from "../ssh/addresses.js";
import { readActiveState, readIdentity, readInvites, readNetMeta, writeInvites, } from "../store/index.js";
function parseDuration(s) {
    const m = /^(\d+)\s*(s|m|h|d)?$/i.exec(s.trim());
    if (!m)
        throw new Error(`bad duration: ${s} (use 30s, 5m, 1h, 1d)`);
...
export async function generateInvite(opts = {}) {
    ...
    if (!identity)
        throw new Error("nagent not initialized (bootstrap should have created identity)");
    ...
    if (!netMeta)
        throw new Error(`active net ${active.activeNetId} not found`);
    ...
    const addrs = opts.addr && opts.addr.length
    ...
    if (!nagentBin)
        throw new Error("cannot resolve nagent bin path (argv[1] empty)");
    const nodeBin = process.execPath;
    const homeForCmd = paths().root;
    const command = `env NAGENT_HOME=${shellQuote(homeForCmd)} ` +
...
function escapeForAuthorizedKeys(s) {
