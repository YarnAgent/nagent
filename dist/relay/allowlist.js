...
export async function loadAllowlist() {
    ...
    for (const netId of netIds) {
        const peers = await readJson(paths().netPeers(netId));
 ...
 */
export function findAllowed(allowlist, nodeName, pubKey) {
    for (const a of allowlist) {
        if (a.nodeName === nodeName && a.pubKey === pubKey)
            return a;
    }
    return null;
}
/** Add an explicit grant. Replaces any prior grant for the same nodeName. */
export async function addGrant(node, pubKey, now = new Date()) {
    if (!node || !pubKey)
        throw new Error("addGrant: node and pubKey are required");
    const cur = await loadExplicit();
    const next = cur.grants.filter((g) => g.node !== node);
    const entry = { node, pubKey, grantedAt: now.toISOString() };
    next.push(entry);
    next.sort((a, b) => a.node.localeCompare(b.node));
    await writeJson(paths().relayAllowlist, { v: FILE_VERSION, grants: next });
    return entry;
}
/** Remove an explicit grant by nodeName. Returns true if something was removed. */
export async function removeGrant(node) {
    const cur = await loadExplicit();
    const next = cur.grants.filter((g) => g.node !== node);
    if (next.length === cur.grants.length)
        return false;
    await writeJson(paths().relayAllowlist, { v: FILE_VERSION, grants: next });
    return true;
}
...
async function loadExplicit() {
    const raw = await readJson(paths().relayAllowlist);
